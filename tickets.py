...
class tickets(interactions.Extension):
  ...
  async def ticketmodal_response(self, ctx: interactions.CommandContext, one):
    ...
    ticketchannel = await ctx.guild.create_channel(
        name=channnelname, type=interactions.ChannelType.GUILD_TEXT, parent_id=694390326911303723, topic=f"Ticket created by {channnelname} | (Creator ID: {ctx.author.id})", permission_overwrites=[
